		...
		<root-section v-bind="wp">
			<div class="header-section">
				<div class="footer-content text-uppercase">
					...
					<ui-link v-if="contacts.phone" v-bind="{
						content: contacts.phone,
						href: useGetTel(contacts.phone),
						color: 'black',
						showArrow: false,
					}" @click.once="trackContact('phone')" class="fw-900 c-phone" />
				</div>
				<nav class="footer-icons">
					...
					<ui-button-icon class="footer-icon" target="blank" @click.once="trackContact('telegram')"
						:href="contacts.telegramLink" icon="telegram" />
				</nav>
			</div>

			<div class="footer-section">
				<nav class="footer-menu">
					<ui-link v-for="(item, ii) in footerMenu" :key="ii" :content=item.content :href=item.href
						color="black" :showArrow=false class="menu-item" />
				</nav>
				<div class="logo-wp">
